...
import { UpdateCompany } from "../redux/actionCreater";
const EditModal = ({ isOpen, onClose, title, company }) => {
  const [agreeterm, agreementchange] = useState(false);
  const dispatch = useDispatch();

  const formik = useFormik({
    validate: validateCompany,
    enableReinitialize: true, // Enable form reinitialize when initialValues change
    initialValues: {
      name: company?.name || "",
      email: company?.email || "",
    ...
    onSubmit: async (values) => {
      dispatch(UpdateCompany(company.id, values));
      formik.resetForm();
      onClose();
    },
  });

 // Reset agreement checkbox when modal closes
 useEffect(() => {
    if (!isOpen) {
